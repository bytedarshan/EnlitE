...
const ServiceDetail = () => {
  ...
  // We duplicate the testimonials multiple times so the infinite scroll never runs out of content
  const marqueeItems = [...service.testimonials, ...service.testimonials, ...service.testimonials, ...service.testimonials];

  return (
    <div className="bg-slate-50 min-h-screen py-10 md:py-16 overflow-hidden">
      
      {/* Injecting custom CSS for the infinite, pause-able marquee */}
      <style>{`
      ...
      {/* --- WALL OF LOVE: INFINITE MARQUEE WITH RESPONSIVE FRAME --- */}
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="max-w-[90rem] mx-auto px-0 sm:px-6 mt-10 mb-10">
        ...
        {/* The Frame / Container - Less padding and smaller border-radius on mobile */}
        <div className="bg-violet-100 sm:rounded-[2.5rem] py-10 md:py-16 border-y sm:border border-violet-200 shadow-inner relative overflow-hidden">
          ...
          <div className="marquee-container overflow-hidden py-2 md:py-4 w-full relative z-10">
            {/* Gradients significantly thinner on mobile (w-8 instead of w-16) */}
            ...
            <div className="animate-marquee-left flex gap-4 md:gap-6 px-2 md:px-3">
              {marqueeItems.map((testimonial, index) => (
                <div 
                  key={`row1-${index}`}
                  // Cards are smaller on mobile (w-[280px]) and have less padding (p-6)
                ...
                >
                  ...
                  <div className="relative z-10">
                    <div className="flex justify-between items-start mb-4 md:mb-6">
                      <div className="flex space-x-1 text-amber-400 text-base md:text-lg">★★★★★</div>
                    ...
                    {/* Text is slightly smaller on mobile (text-sm md:text-base) */}
